 ...
 * @author Trung
 */
@WebServlet(name = "SearchSPControll", urlPatterns = {"/search"})
public class SearchHomeControll extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        FilterHomeDAO daoSearch = new FilterHomeDAO();
        SanPhamDAO daoSP = new SanPhamDAO();
        String cateID = request.getParameter("txt");
        
        List<Nhomhang> listNH = daoSP.getNhomHang();
        List<Loaihang> listLH = daoSP.getLoaiHang();
        List<Sanpham> listSP = daoSearch.getSearchSanPham(cateID);
        
        request.setAttribute("ListNH", listNH);
        request.setAttribute("ListLH", listLH);
        request.setAttribute("ListSP", listSP);
        request.setAttribute("keyWord", cateID);
